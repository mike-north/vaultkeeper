...
{<!-- -->AuthorizationDeniedError<!-- -->} If `token` is invalid or was not created by this vault instance, or if it is a signing-key token — a signing key carries no secret and cannot be read through `getSecret()` (use `sign()`<!-- -->).
