...
describe('resolvePlaceholders', () => {
  describe('single-secret mode (string)', () => {
    it('replaces {{secret}} with the secret value', () => {
      expect(resolvePlaceholders('Bearer {{secret}}', 'tok123')).toBe('Bearer tok123')
    })
    ...
    it('returns the original string when no placeholder is present', () => {
      expect(resolvePlaceholders('no placeholder', 'secret')).toBe('no placeholder')
    })

    it('leaves {{secret:name}} placeholders unreplaced', () => {
      expect(resolvePlaceholders('{{secret:apiKey}}', 'val')).toBe('{{secret:apiKey}}')
    })
  })
  ...
  describe('named-secret mode (Record)', () => {
    it('replaces {{secret:name}} with the corresponding value', () => {
      ...
      expect(resolvePlaceholders('key={{secret:apiKey}} pass={{secret:dbPass}}', secrets)).toBe(
        'key=key123 pass=pass456',
      )
    })

    it('replaces multiple occurrences of the same name', () => {
      const secrets = { tok: 'abc' }
      expect(resolvePlaceholders('{{secret:tok}}-{{secret:tok}}', secrets)).toBe('abc-abc')
    })

    it('returns the original string when no named placeholder is present', () => {
      const secrets = { tok: 'abc' }
      expect(resolvePlaceholders('no placeholder', secrets)).toBe('no placeholder')
    })
    ...
    it('throws VaultError for unknown secret name', () => {
      ...
      expect(() => resolvePlaceholders('{{secret:unknown}}', secrets)).toThrow(VaultError)
      expect(() => resolvePlaceholders('{{secret:unknown}}', secrets)).toThrow(
        /Unknown secret name.*unknown/,
      )
    })

    it('includes available names in error message', () => {
      const secrets = { apiKey: 'key', dbPass: 'pass' }
      expect(() => resolvePlaceholders('{{secret:missing}}', secrets)).toThrow(/apiKey, dbPass/)
    })
  })
})
