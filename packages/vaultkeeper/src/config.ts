...
const CONFIG_REMEDIATION_HINT =
  "Fix the file, install @vaultkeeper/cli and run 'vaultkeeper config init --force' to overwrite " +
  'it with a valid config, or repair/replace it programmatically via this library (pass an ' +
  'explicit `config` or `configDir`, or write a valid config.json yourself).'
